...
of money each member has lent or borrowed. The user should be able to add/delete/edit requests. 
6. Once the event is created, the other members will be notified and the event will show up in their apps. 
7. Each user can either accept/reject a request-notification or delete his participant in the event. The 
status of each request will be registered and visible on both the owner's and the member's events.  
8. Actions taken on the event by the issuer and the members should be logged in text in the event and 
